...
import type { KaspiResult } from "@/lib/pos";

export type KaspiPayload = {
  orderId: string;
  orderNo: number;
  amount: number;
 ...
 * и сохраняет результат в заказе.
 */
export async function runFiscalization(
  ...
  if (!settings?.kaspi_enabled)
    return { status: "not_configured", message: "Kaspi Касса выключена в настройках" };
  if (!token) return { status: "not_configured", message: "Не задан токен устройства Kaspi" };
  if (!settings.kaspi_api_url)
    return { status: "not_configured", message: "Не указан адрес Kaspi API" };
